<?php
  ...
  if(isset($_REQUEST['mode']) && $_REQUEST['mode']!="")
  {
  	...
  	if($service=="product_data")
  	{
  		$order_month=isset($_REQUEST['product_month'])?$_REQUEST['product_month']:date("m");
  		$order_year=isset($_REQUEST['product_year'])?$_REQUEST['product_year']:date("Y");
  		$order_data_r=$db->rp_getData("product_inquiry","*","MONTH(created_date)='".$order_month."' AND YEAR(created_date)='".$order_year."' AND isDelete=0","",1);
  		if($order_data_r)
  		{
  			$orders=array();
  			while($order_data=mysqli_fetch_assoc($order_data_r))
  			{
  				// print_r($order_data['name']);exit;
  				...
  				{
  					print_r($order_timestamp);exit;
  					// print_r($orders[$order_timestamp]['email']);exit;
