<?php
   ...
   $page_hierarchy=array(array("link"=>"dashboard.php","title"=>"Dashboard"),array("link"=>"nik_manage.php","title"=>"Manage Blog"),array("link"=>"nik_crud.php","title"=>"Add/Edit Blog"));
   $success_msg=array();
   $error_msg=array();
   // PAGE DECLARATION
   
   // INCLUDE CLASS
   include('../include/class.nik.php');
   
   $nik_obj=new Blog();
   $mode=isset($_REQUEST['mode'])?$_REQUEST['mode']:"a";
   
   // Variable Definations
   $id='';$cid='';$nik_title='';$slug='';$image_path="";$description="";$add_date="";$file_upload="";$author_name="";
   ...
   $params['nik_title']=trim($db->clean($_REQUEST['nik_title']));
   $params['slug']=$db->rp_createSlug($_REQUEST['nik_title']);
   ...
   if($mode=='a')
	{
	   ...
	   $reply=$nik_obj->insert($params,"",$_FILES);
		if($reply['ack']==1)
		{
			$success_msg[]=$reply['ack_msg'];
            $_SESSION['success_msg']= $success_msg;
            $db->rp_location("nik_manage.php");
		}
		else
		{
   	...
   	{
   		$reply=$nik_obj->update($params,$params['nik_title'],$params['id'],$_FILES);
   		if($reply['ack']==1)
   		{
   			$success_msg[]=$reply['ack_msg'];
               $_SESSION['success_msg']= $success_msg;
   	...
   	else if($mode=='d')
   	{
   		...
   		$reply=$nik_obj->delete(array("key"=>"id","value"=>$params['id']));
   		if($reply['ack']==1)
   		{
   		    $success_msg[]=$reply['ack_msg'];
               $_SESSION['success_msg']= $success_msg;
   	...
   	else if($mode=='ac')
   	{
   	   //
   		...
   		$reply=$nik_obj->active(array("key"=>"id","value"=>$params['id'],"status"=>$params['status']));
   		if($reply['ack']==1)
   		{
   		    $success_msg[]=$reply['ack_msg'];
               $_SESSION['success_msg']= $success_msg;
                           ...
                           <div class="col-md-6">
                              <div class="portlet light bordered">
                                 ...
                                 <div class="portlet-body form">
                                    ...
                                    <div class="form-group">
                                       <label for="nik_title">Blog Title <code>*</code></label>
                                       <input class="form-control " value="<?php echo $nik_title;  ?>" name="nik_title" id="nik_title" type="text"  data-validation="required,length"   data-validation-length="min3" data-validation-error-msg="Enter atleast 3 Characters." >
                                    ...
                                    <div class="form-actions noborder">
                                       ...
                                       <button type="button" class="btn btn-primary" onClick="window.location.href='nik_manage.php'">Back</button>
                                    </div>
                                 </div>
                              </div>
                           </div>
