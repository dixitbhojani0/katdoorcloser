<?php
  class AlterImage extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="alt_img";
  	public $primary_column="id";
  	public $unique_column="";
  	// Public Varibale
  	public $id='';public $pid='';public $image_path='';public $display_order='';public $adate=''; 
  	public $valid_keys=array("id","pid","image_path","old_image_path","adate");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="")
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Alter Image fetched successfully!!","ack_msg"=>"Alter Image fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Alter image can not be fetch!","ack_msg"=>"Alter image can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Alter image cannot be fetch!","ack_msg"=>"Alter image cannot be fetch!");
  			return $reply;
  		}
  		
  	}
  
  	function insert($detail=array(),$dup_check_array=array(),$item,$file)
  	{
  		//$this->db->rp_delete($this->ctable,"pid  ='".$detail['pid']."'",0);
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countAlterImage($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						...
  						{
  							$current_item=$item[$i];
  							$detail['image_path']=$current_item['image_path'];
  							...
  							if (isset($file["image_path"])) 
  							{
  								...
  								if(!in_array($extension, $allowedExts)){
  									...
  									$fileName = 'alter_img'.substr(sha1(time()), 0, 6).".".$extension;
                    // echo $fileName;exit;
  									$tempPath="../images/alter_image/".$fileName;
  							...
  							if($inserted_id!=0)
  							{
  								...
  								$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Alter image inserted successfully!!","ack_msg"=>"Alter image inserted successfully!!","pid"=>$detail['pid']);
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Alter image  can't be inserted!!","ack_msg"=>"Alter image can't be inserted!!");
  								return $reply;
  							}
  		...
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Alter image detail cannot be fetch!","ack_msg"=>"Alter image detail cannot be fetch!");
  			return $reply;
  		}
  	}
  		...
  		if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countAlterImage($this->primary_column,$primary_key);
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							if (isset($file["image_path"]) && $file["image_path"]['size']!=0) 
  							{
  									...
  									$tempPath="../images/alter_image/".$file['image_path']['name'];
  									move_uploaded_file($file["image_path"]['tmp_name'],$tempPath);
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Alter image  updated successfully!!","ack_msg"=>"Alter image updated successfully!!","pid"=>$detail['pid']);
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Alter image  cannot be updated!!","ack_msg"=>"Alter image cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countAlterImage($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				$pid=$this->db->rp_getValue($this->ctable,"pid",$delete_array['key']."=".$delete_array['value'],0);
  				$this->db->rp_delete($this->ctable,$delete_array['key']."=".$delete_array['value'],0);
              	$reply=array("ack"=>1,"developer_msg"=>"Alter image deleted successfully!!","ack_msg"=>"Alter image deleted successfully!!","pid"=>$pid);
              	return $reply;
              }
              else
              {
  	...
  	function countAlterImage($key,$value)
  	{		
  		$countAlterImage = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countAlterImage;	
  	}
  	function duplicateProduct($key,$value,$primary_key)
  	{		
  		$countAlterImage = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countAlterImage;	
  	}
