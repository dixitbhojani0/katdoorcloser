<?php
  class certificate extends Functions
  {
    public $detail=array();
    public $db,$rights;
    public $ctable="certificate";
    ...
    public $valid_keys=array("id","cid","image_path","old_image_path","title","description","adate");
    ...
    function view($where="",$required_columns=array(),$orderby="",$limit="",$isSubCategoryRequired=false,$isProductRequired=false)
    {
      if($this->rights['view_flag']==1)
      {
        ...
        if($result_r)
        {
          ...
          $reply=array("ack"=>1,"developer_msg"=>"certificate detail fetched successfully!!","ack_msg"=>"certificate detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
          return $reply;
        }
        else
        {
  
          $reply=array("ack"=>0,"developer_msg"=>"certificate detail can not be fetch!","ack_msg"=>"certificate detail can not be fetch!");
          return $reply;
        }
      }
      else
      {
        ...
        $reply=array("ack"=>0,"developer_msg"=>"certificate detail cannot be fetch!","ack_msg"=>"certificate detail cannot be fetch!");
        return $reply;
      }
          ...
          if(!empty($detail))
          {
            ...
            if($count<=0)
            {
              ...
              if(!in_array($extension, $allowedExts)){
                ...
                $tempPath="../images/certificate/".$file['image_path']['name'];
              ...
              if($inserted_id!=0)
              {
                ...
                $reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"certificate Detail inserted successfully!!","ack_msg"=>"certificate Detail inserted successfully!!");
                return $reply;
              }
              else
              {
                $reply=array("ack"=>0,"developer_msg"=>"certificate Detail can't be inserted!!","ack_msg"=>"certificate Detail can't be inserted!!");
                return $reply;
              }
            } 
            else
          ...
          if(!empty($detail)&& $primary_key!="")
          {
            ...
            if($count>=1)
            {
              // $count=$this->duplicateProduct($this->unique_column,$unique_key,$primary_key);
                ...
                if($isUpdated)
                {
                      $reply=array("ack"=>1,"developer_msg"=>"certificate Detail updated successfully!!","ack_msg"=>"certificate Detail updated successfully!!");
                  return $reply;
                }
                else
                {
                  $reply=array("ack"=>0,"developer_msg"=>"certificate Detail cannot be updated!!","ack_msg"=>"certificate Detail cannot be updated!!");
                  return $reply;
                }
    ...
    function delete($delete_array)
    {
      if($this->rights['delete_flag']==1)
      {
        $count=$this->countCategory($delete_array['key'],$delete_array['value']);
          ...
          $values=array("isDelete"=>1);
                ...
                $this->db->rp_update("certificate",$values,"cid=".$delete_array['value']."",0);
                
          $reply=array("ack"=>1,"developer_msg"=>"certificate deleted successfully!!","ack_msg"=>"certificate deleted successfully!!");
                return $reply;
              }
              else
              {
      ...
      {
        $this->db->rp_location("access_denied.php?msg=insert");
        $reply=array("ack"=>0,"developer_msg"=>"certificate cannot be fetch!","ack_msg"=>"certificate cannot be fetch!");
        return $reply;
      }   
    }
    ...
    function active($active_array)
    {
      if($this->rights['update_flag']==1)
      {
          ...
          if($count>=1)
          {
            ...
            $this->db->rp_update("certificate",array("isActive"=>$active_array['status']),"cid=".$active_array['value']."",0);
            
            $reply=array("ack"=>1,"developer_msg"=>"certificate status updated successfully!!","ack_msg"=>"certificate status updated successfully!!");
            return $reply;
          }
          else
          {
