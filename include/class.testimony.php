<?php
  class Testimony extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="testimony";
  	public $primary_column="id";
  	public $unique_column="name";
  	// Public Varibale
  	public $id='';public $name='';public $slug='';public $designation='';public $message='';public $image_path='';public $adate=''; 
  	public $valid_keys=array("id","name","slug","designation","message","image_path","old_image_path","adate");
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countTestimony($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/testimonials/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Testimonial Detail inserted successfully!!","ack_msg"=>"Testimonial Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Testimonial Detail can't be inserted!!","ack_msg"=>"Testimonial Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
  	...
  	function update($detail=array(),$unique_key,$primary_key,$file)
  	{
  		if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countTestimony($this->primary_column,$primary_key);
  					if($count>=1)
  					{
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Gallery Detail updated successfully!!","ack_msg"=>"Testimonial Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Testimonial Detail cannot be updated!!","ack_msg"=>"Testimonial Detail cannot be updated!!");
  								return $reply;
  							}
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Testimonial Detail cannot be fetch!","ack_msg"=>"Gallery Detail cannot be fetch!");
  			return $reply;
  		}
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countTestimony($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Testimonial deleted successfully!!","ack_msg"=>"Testimonial deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Testimonial cannot be fetch!","ack_msg"=>"Testimonial cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  					...
  					$count=$this->countTestimony($active_array['key'],$active_array['value']);
  					if($count>=1)
  					{
  						$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
  						$reply=array("ack"=>1,"developer_msg"=>"Testimonial status updated successfully!!","ack_msg"=>"Testimonial status updated successfully!!");
  						return $reply;
  					}
  					else
  					{
  	...
  	function countTestimony($key,$value)
  	{		
  		$countTestimony = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countTestimony;	
  	}
  	function duplicateTestimony($key,$value,$primary_key)
  	{		
  		$countTestimony = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countTestimony;	
  	}
