<?php
  class PageTable extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="page_table";
  	public $primary_column="id";
  	public $unique_column="page_title";
  	// Public Varibale
  	public $id='';public $page_title='';public $page_slug='';public $page_count='';public $page_urls='';public $adate=''; 
  	public $valid_keys=array("id","page_title","page_slug","page_count","page_urls","adate");
  	...
  	function insert($detail=array(),$dup_check_array=array())
  	{
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			{ 
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countPageTable($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					if($count<=0)
  					{
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"developer_msg"=>"Page detail inserted successfully!!","ack_msg"=>"Page detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Page deatil can't be insert!!","ack_msg"=>"Page deatil can't be insert!!");
  							return $reply;
  						}
  					}
  					else
  		...
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Page detail cannot be fetch!","ack_msg"=>"Page detail cannot be fetch!");
  			return $reply;
  		}
  	}
  ...
  function update($detail=array(),$unique_key,$primary_key)
  		...
  		if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countPageTable($this->primary_column,$primary_key);
  					if($count>=1)
  					{
  						$count=$this->duplicatePagetable($this->unique_column,$unique_key,$primary_key);
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
  								$reply=array("ack"=>1,"developer_msg"=>"Page detail updated successfully!!","ack_msg"=>"Page detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Page detail cannot be updated!!","ack_msg"=>"Page detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array,$detail)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              $count=$this->countPageTable($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				
              	$this->db->rp_delete($this->ctable,$delete_array['key']."=".$delete_array['value'],0);
              	$reply=array("ack"=>1,"developer_msg"=>"Page detail deleted successfully!!","ack_msg"=>"Page detail deleted successfully!!");
              	return $reply;
              }
              else
              {
      ...
      function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				$count=$this->countPageTable($active_array['key'],$active_array['value']);
  				if($count>=1)
  				{
  					$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
  					$reply=array("ack"=>1,"developer_msg"=>"Page deatil status updated successfully!!","ack_msg"=>"Page status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
  	...
  	function countPageTable($key,$value)
  	{		
  		$countPageTable = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countPageTable;	
  	}
  	function duplicatePagetable($key,$value,$primary_key)
  	{		
  		$countPageTable = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countPageTable;	
  	}
