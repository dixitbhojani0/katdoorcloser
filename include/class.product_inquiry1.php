<?php
  class Services extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="services";
  	public $primary_column="id";
  	public $unique_column="name";
  	// Public Varibale
  	public $id='';public $name='';public $cid='';public $slug='';public $meta_descr='';public $description='';public $image_path='';public $old_image_path='';public $adate='';  public $product_code='';
  	public $valid_keys=array("id","cid","name","slug","meta_descr","description","image_path","old_image_path","adate");
  	...
  	function insert($detail=array(),$dup_check_array=array(),$file)
  	{
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			// {
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countServices($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						if (isset($file["image_path"])) 
  						{
  							...
  							$tempPath="../images/services/".$fileName;
  						...
  						/* file upload */
  						// if (isset($file["file_upload"])) 
  						// {
  						// $allowedExts = array("pdf","txt","docx");
  						// $temp = explode(".", $file["file_upload"]["name"]);
  						// $extension = end($temp);
  
  						// if($file["file_upload"]["error"]>0) {
  						// 	$error .= "Error opening the file. ";
  						// }
  						// if($file["file_upload"]["type"]=="application/x-msdownload"){
  						// 	$error .= "Mime type not allowed. ";
  						// }
  						// if(!in_array($extension, $allowedExts)){
  						// 	$error .= "Extension not allowed. ";
  						...
  						// 	$fileName  = $this->db->clean($file["file_upload"]["name"]);
  						// 	$fileSize  = round($file["file_upload"]["size"]); // BYTES
  						// 	//echo $fileSize ;exit;
  						// 	$adate   = date('Y-m-d H:i:m');
  						...
  						// 	$tempPath="../images/services/download_pdf/".$file['file_upload']['name'];
  						// 	move_uploaded_file($file["file_upload"]['tmp_name'],$tempPath);
  						// 	$detail['file_upload']=$file['file_upload']['name'];
  						// 	unset($detail['old_file_upload']);
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Services Detail inserted successfully!!","ack_msg"=>"Services Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Services Detail can't be inserted!!","ack_msg"=>"Services Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
  		...
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Services Detail cannot be fetch!","ack_msg"=>"Services Detail cannot be fetch!");
  			return $reply;
  		}
  	}
  		...
  		if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countServices($this->primary_column,$primary_key);
  					if($count>=1)
  					{
  
  						$count=$this->duplicateServices($this->unique_column,$unique_key,$primary_key);
  						if($count<=0)
  						{
  							if (isset($file["image_path"]))
  							{	
  							...
  							/* file upload */
  								...
  								// {
  								// 	//$fileName=$detail['old_image_path'];
  								// 	$detail['file_upload']=$detail['old_file_upload'];
  								// 	unset($detail['old_file_upload']);
  								// }
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Services Detail updated successfully!!","ack_msg"=>"Services Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Services Detail cannot be updated!!","ack_msg"=>"Services Detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countServices($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				$this->db->rp_delete($this->ctable,$delete_array['key']."=".$delete_array['value'],0);
              	$reply=array("ack"=>1,"developer_msg"=>"Services deleted successfully!!","ack_msg"=>"Services deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Services cannot be fetch!","ack_msg"=>"Services cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		...
  		if($this->rights['update_flag']==1)
  		{
  			...
  			$cat_id=$this->db->rp_getValue($this->ctable,"cid","id=".$active_array['id']."",0);
  			/*$sub_id=$this->db->rp_getValue($this->ctable,"sid","id=".$active_array['id']."",0);	*/
  	
  			$cat_isActive=$this->db->rp_getValue("service_category","isActive","id=".$cat_id."",0);
  				
  				if($cat_isActive==1)			
  				{ 
  					$count=$this->countServices($active_array['key'],$active_array['value']);
  					if($count>=1)
  					{
  	...
  	function countServices($key,$value)
  	{		
  		$countServices = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countServices;	
  	}
  	function duplicateServices($key,$value,$primary_key)
  	{		
  		$countServices = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countServices;	
  	}
