<?php
  class pages extends Functions
  {
  	...
  	public $unique_column="pages_title";
  	// Public Varibale

  	public $id='';public $pages_title='';public $slug='';public $description='';public $adate='';  public $author_name='';

  	public $valid_keys=array("id","cid","pages_title","slug","description","add_info","author_name","image_path","old_image_path","adate");
      ...
      if($this->rights['view_flag']==1)
  		{             
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"pages detail fetched successfully!!","ack_msg"=>"pages detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{  
  				$reply=array("ack"=>0,"developer_msg"=>"pages detail can not be fetch!","ack_msg"=>"pages detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"pages detail cannot be fetch!","ack_msg"=>"pages detail cannot be fetch!");
  			return $reply;
  		}  		
  	}
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			// {
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countpages($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						if (isset($file["image_path"])) 
  						{
  							...
  							$fileName = 'pages_img_'.substr(sha1(time()), 0, 6).".".$extension;
  							$tempPath="../images/pages/".$fileName;
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"pages Detail inserted successfully!!","ack_msg"=>"pages Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"pages Detail can't be inserted!!","ack_msg"=>"pages Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
      ...
      if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countpages($this->primary_column,$primary_key);
  					if($count>=1)
  					{
              $count=$this->duplicateProduct($this->unique_column,$unique_key,$primary_key);
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"pages Detail updated successfully!!","ack_msg"=>"pages Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"pages Detail cannot be updated!!","ack_msg"=>"pages Detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countpages($delete_array['key'],$delete_array['value']);
        if($count>=1)
        {
  				$values=array("isDelete"=>1);
  				$this->db->rp_update($this->ctable,$values,"id='".$delete_array['value']."'",0);
        	$reply=array("ack"=>1,"developer_msg"=>"pages deleted successfully!!","ack_msg"=>"pages deleted successfully!!");
        	return $reply;
        }
        else
        {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"pages cannot be fetch!","ack_msg"=>"pages cannot be fetch!");
  			return $reply;
  		}		
  	} 
  	...
  	{   
  		if($this->rights['update_flag']==1)
  		{ 
				$count=$this->countpages($active_array['key'],$active_array['value']);
				if($count>=1)
				{
					$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
					$reply=array("ack"=>1,"developer_msg"=>"pages status updated successfully!!","ack_msg"=>"pages status updated successfully!!");
					return $reply;
				}
				else
				{
  	...
  	function countpages($key,$value)
  	{		
  		$countpages = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countpages;	
  	}
  	function duplicateProduct($key,$value,$primary_key)
  	{		
  		$countpages = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countpages;	
  	}
