<?php
  class SubCategory extends Functions
  {
  	...
  	// Public Varibale
  	public $id='';public $cid='';public $name='';public $slug='';public $meta_descr='';public $description='';public $image_path='';public $adate=''; 
  	public $valid_keys=array("id","cid","name","slug","meta_descr","description","image_path","adate");
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countSubCategory($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Sub Category inserted successfully!!","ack_msg"=>"Sub Category inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Sub Category  can't be inserted!!","ack_msg"=>"Sub Category can't be inserted!!");
  							return $reply;
  						}
  					}
  					else
  		...
  		if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countSubCategory($this->primary_column,$primary_key);
  					if($count>=1)
  					{
  						$count=$this->duplicateSubCategory($this->unique_column,$unique_key,$primary_key);
  						if($count<=0)
  						{
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countSubCategory($delete_array['key'],$delete_array['value']);
        if($count>=1)
        {
    ...
    function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{  
				$count=$this->countSubCategory($active_array['key'],$active_array['value']);
				if($count>=1)
				{
  	...
  	function countSubCategory($key,$value)
  	{		
  		$countSubCategory = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countSubCategory;	
  	}
  	function duplicateSubCategory($key,$value,$primary_key)
  	{		
  		$countSubCategory = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countSubCategory;	
  	}
