<?php
  class SubCategory extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="sub_category";
  	public $primary_column="id";
  	public $unique_column="name";
  	// Public Varibale
  	public $id='';public $name='';public $cid='';public $slug='';public $packing='';public $description='';public $image_path='';public $adate='';  public $product_code='';
  	public $valid_keys=array("id","cid","name","product_code","slug","packing","description","meta_descr","image_path","old_image_path","adate");
      ...
      if($this->rights['view_flag']==1)
      	...
      	$required_columns=$this->getRequiredColumns($required_columns);
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Sub-Category detail fetched successfully!!","ack_msg"=>"Sub-Category detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Sub-Category detail can not be fetch!","ack_msg"=>"Sub-Category detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Sub-Category detail cannot be fetch!","ack_msg"=>"Sub-Category detail cannot be fetch!");
  			return $reply;
  		}
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						if (isset($file["image_path"])) 
  						{
  							...
  							$fileName = 'subcat_limg_'.substr(sha1(time()), 0, 6).".".$extension;
  							$tempPath="../images/subcategory/".$fileName;
  						...
  						/* short image upload */
  						// if (isset($file2["short_image_path"])) 
  						// {
  						// 	$file_tmp  = $file2['short_image_path']['tmp_name'];
  						// 	$allowedExts = array("jpg","jpeg","png","gif","JPG","JPEG");
  						// 	$temp = explode(".", $file2["short_image_path"]["name"]);
  						// 	$extension = end($temp);
  
  						// 	if($file2["short_image_path"]["error"]>0) {
  						// 		$error .= "Error opening the file. ";
  						// 	}
  						// 	if($file2["short_image_path"]["type"]=="application/x-msdownload"){
  						// 		$error .= "Mime type not allowed. ";
  						// 	}
  						// 	if(!in_array($extension, $allowedExts)){
  						// 		$error .= "Extension not allowed. ";
  						...
  						// 	$fileName  = $this->db->clean($file2["short_image_path"]["name"]);
  						// 	$fileSize  = round($file2["short_image_path"]["size"]); // BYTES
  						// 	//echo $fileSize ;exit;
  						// 	$adate   = date('Y-m-d H:i:m');
  
  						// 	$extension = end(explode(".", $fileName));
  						
  						// 	$fileName = 'subcat_simg_'.substr(sha1(time()), 0, 6).".".$extension;
  						// 	$tempPath="../images/subcategory/".$fileName;
  						// 	move_uploaded_file($file_tmp,$tempPath);
  						// 	$detail['short_image_path']=$fileName;
  						// 	unset($detail['old_short_image_path']);
  						// }
  						/* short image upload */
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Sub-Category Detail inserted successfully!!","ack_msg"=>"Sub-Category Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Sub-Category Detail can't be inserted!!","ack_msg"=>"Sub-Category Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							// 	if (isset($file2["short_image_path"]) && $file2["short_image_path"]['size']!=0) 
  							// 	{
  							// 		$file_tmp  = $file2['short_image_path']['tmp_name'];
  							// 		$allowedExts = array("jpg","jpeg","png","gif","JPG","JPEG");
  							// 		 $temp = explode(".", $file2["short_image_path"]["name"]);
  							...
  							// 	{
  							// 		//$fileName=$detail['old_image_path'];
  							// 		$detail['short_image_path']=$detail['old_short_image_path'];
  							// 		unset($detail['old_short_image_path']);
  							// 	}
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Sub-Category Detail updated successfully!!","ack_msg"=>"Sub-Category Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Sub-Category Detail cannot be updated!!","ack_msg"=>"Sub-Category Detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$reply=array("ack"=>1,"developer_msg"=>"Sub-Category deleted successfully!!","ack_msg"=>"Sub-Category deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Sub-Category cannot be fetch!","ack_msg"=>"Sub-Category cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	{  
  		...
  		{ 
            ...
            $this->db->rp_update("product",array("isActive"=>$active_array['status']),"sid=".$active_array['value'],0);

						$reply=array("ack"=>1,"developer_msg"=>"Sub-Category status updated successfully!!","ack_msg"=>"Sub-Category status updated successfully!!");
						return $reply;
					}
					else
					{
