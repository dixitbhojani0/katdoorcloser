<?php
  class Team extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="our_team";
  	public $primary_column="id";
  	public $unique_column="name";
  	// Public Varibale
  	public $id='';public $name='';public $slug='';public $meta_descr='';public $description='';public $image_path='';public $old_image_path='';public $adate=''; 
  	public $valid_keys=array("id","name","image_path","old_image_path","designation","description","adate","fb_link","twitter_link","insta_link","linkedin_link","google_link");
      ...
      if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Team detail fetched successfully!!","ack_msg"=>"Team detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{  
  				$reply=array("ack"=>0,"developer_msg"=>"Team detail can not be fetch!","ack_msg"=>"Team detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Team detail cannot be fetch!","ack_msg"=>"Team detail cannot be fetch!");
  			return $reply;
  		}
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countTeam($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/team/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Team Detail inserted successfully!!","ack_msg"=>"Team Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Team Detail can't be inserted!!","ack_msg"=>"Team Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
  		...
  		if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countTeam($this->primary_column,$primary_key);
  					if($count>=1)
  					{
  						// $count=$this->duplicateProduct($this->unique_column,$unique_key,$primary_key);
  							...
  							if($isUpdated)
  							{
  						    $reply=array("ack"=>1,"developer_msg"=>"Team Detail updated successfully!!","ack_msg"=>"Team Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Team Detail cannot be updated!!","ack_msg"=>"Team Detail cannot be updated!!");
  								return $reply;
  							}
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countTeam($delete_array['key'],$delete_array['value']);
        if($count>=1)
        { 
  				$values=array("isDelete"=>1);
        	$this->db->rp_update($this->ctable,$values,"id=".$delete_array['value']."",0); 
              	
  				$reply=array("ack"=>1,"developer_msg"=>"Team deleted successfully!!","ack_msg"=>"Team deleted successfully!!");
              	return $reply;
        }
        else
        {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Team cannot be fetch!","ack_msg"=>"Team cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				$count=$this->countTeam($active_array['key'],$active_array['value']);
  				if($count>=1)
  				{
  					...
  					$this->db->rp_update("infrastructure_gallery",array("isActive"=>$active_array['status']),"cid=".$active_array['value']."",0);
  					
  					$reply=array("ack"=>1,"developer_msg"=>"Team status updated successfully!!","ack_msg"=>"Team status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
  	...
  	function countTeam($key,$value)
  	{		
  		$countTeam = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countTeam;	
  	}
  	function duplicateCategory($key,$value,$primary_key)
  	{		
  		$countTeam = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countTeam;	
  	}
