<?php
  class Dealer extends Functions
  {
  	...
  	public $unique_column="company_name";
  	// Public Varibale
  	public $id='';public $name='';public $slug='';public $meta_descr='';public $description='';public $image_path='';public $image=''; 
  	public $valid_keys=array("id","company_name","slug","address","country","state","city","pincode","email","mobile_no","website","adate");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="",$isSubCategoryRequired=false,$isProductRequired=false)
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Category detail fetched successfully!!","ack_msg"=>"Category detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Category detail can not be fetch!","ack_msg"=>"Category detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/category/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Category inserted successfully!!","ack_msg"=>"Category inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Category  can't be inserted!!","ack_msg"=>"Category can't be inserted!!");
  							return $reply;
  						}
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Category detail updated successfully!!","ack_msg"=>"Category detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Category detail cannot be updated!!","ack_msg"=>"Category detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$reply=array("ack"=>1,"developer_msg"=>"Category deleted successfully!!","ack_msg"=>"Category deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Category cannot be fetch!","ack_msg"=>"Category cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				if($count>=1)
  				{
  					...
  					$reply=array("ack"=>1,"developer_msg"=>"Category status updated successfully!!","ack_msg"=>"Category status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
