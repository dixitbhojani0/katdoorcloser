<?php
  class Scheme extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="scheme";
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="",$isSubCategoryRequired=false,$isProductRequired=false)
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Scheme detail fetched successfully!!","ack_msg"=>"Scheme detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Scheme detail can not be fetch!","ack_msg"=>"Category detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Scheme detail cannot be fetch!","ack_msg"=>"Scheme detail cannot be fetch!");
  			return $reply;
  		}
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/scheme/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Scheme inserted successfully!!","ack_msg"=>"Scheme inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Scheme  can't be inserted!!","ack_msg"=>"Scheme can't be inserted!!");
  							return $reply;
  						}
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Scheme detail updated successfully!!","ack_msg"=>"Scheme detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Scheme detail cannot be updated!!","ack_msg"=>"Scheme detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$reply=array("ack"=>1,"developer_msg"=>"Scheme deleted successfully!!","ack_msg"=>"Scheme deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Scheme cannot be fetch!","ack_msg"=>"Scheme cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				if($count>=1)
  				{
  					...
  					$reply=array("ack"=>1,"developer_msg"=>"Scheme status updated successfully!!","ack_msg"=>"Scheme status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
