<?php
  class ClientSlider extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="client_slider";
  	public $primary_column="id";
  	public $unique_column="name";
  	// Public Varibale
  	public $id='';public $name='';public $slug='';public $meta_descr='';public $description='';public $image_path='';public $adate=''; 
  	public $valid_keys=array("id","name","slug","meta_descr","description","image_path","old_image_path","adate");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="")
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Quality detail fetched successfully!!","ack_msg"=>"Quality detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Quality detail can not be fetch!","ack_msg"=>"Quality detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Quality detail cannot be fetch!","ack_msg"=>"Quality detail cannot be fetch!");
  			return $reply;
  		}
  	...
  	function insert($detail=array(),$dup_check_array=array(),$file)
  	{
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/client_silder/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Client Detail inserted successfully!!","ack_msg"=>"Client Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Client Detail can't be inserted!!","ack_msg"=>"Client Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
  		...
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Client Detail cannot be fetch!","ack_msg"=>"Client Detail cannot be fetch!");
  			return $reply;
  		}
  	}
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Client Detail updated successfully!!","ack_msg"=>"Client Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Client Detail cannot be updated!!","ack_msg"=>"Client Detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
