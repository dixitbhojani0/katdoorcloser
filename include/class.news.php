<?php
  class News extends Functions
  {
  	...
  	// Public Varibale
  	public $id='';public $title='';public $slug='';public $description='';public $image_path='';public $adate=''; public $news_date;
  	public $valid_keys=array("id","title","slug","description","image_path","old_image_path","adate","news_date");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="")
  	{
  		...
  		{
             // $detail['news_date']=date("d-m-Y",strtotime($detail['news_date']));
  			$result=array();
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"News detail fetched successfully!!","ack_msg"=>"News detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"News detail can not be fetch!","ack_msg"=>"News detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"News detail cannot be fetch!","ack_msg"=>"News detail cannot be fetch!");
  			return $reply;
  		}
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/news/".$file['image_path']['name'];
  							...
  							$detail['news_date']=date("Y-m-d",strtotime($detail['news_date']));
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"News inserted successfully!!","ack_msg"=>"News inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"News  can't be inserted!!","ack_msg"=>"News can't be inserted!!");
  							return $reply;
  						}
  						}
  					else
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						...
  						if($isUpdated)
  						{
  							$reply=array("ack"=>1,"developer_msg"=>"News detail updated successfully!!","ack_msg"=>"News detail updated successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"News detail cannot be updated!!","ack_msg"=>"News detail cannot be updated!!");
  							return $reply;
  						}
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$reply=array("ack"=>1,"developer_msg"=>"News deleted successfully!!","ack_msg"=>"News deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"News cannot be fetch!","ack_msg"=>"News cannot be fetch!");
  			return $reply;
  		}		
  	}
