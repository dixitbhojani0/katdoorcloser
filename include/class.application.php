<?php
  class application extends Functions
  {
  	...
  	public $valid_keys=array("id","image_path","old_image_path","name","description","adate");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="",$isSubCategoryRequired=false,$isProductRequired=false)
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"application detail fetched successfully!!","ack_msg"=>"application detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"application detail can not be fetch!","ack_msg"=>"application detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"application detail cannot be fetch!","ack_msg"=>"application detail cannot be fetch!");
  			return $reply;
  		}
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/application/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"application Detail inserted successfully!!","ack_msg"=>"application Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"application Detail can't be inserted!!","ack_msg"=>"application Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						// $count=$this->duplicateProduct($this->unique_column,$unique_key,$primary_key);
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"application Detail updated successfully!!","ack_msg"=>"application Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"application Detail cannot be updated!!","ack_msg"=>"application Detail cannot be updated!!");
  								return $reply;
  							}
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$this->db->rp_update("application",$values,"cid=".$delete_array['value']."",0);
              	
  				$reply=array("ack"=>1,"developer_msg"=>"application deleted successfully!!","ack_msg"=>"application deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"application cannot be fetch!","ack_msg"=>"application cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				if($count>=1)
  				{
  					...
  					$this->db->rp_update("application",array("isActive"=>$active_array['status']),"cid=".$active_array['value']."",0);
  					
  					$reply=array("ack"=>1,"developer_msg"=>"application status updated successfully!!","ack_msg"=>"application status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
