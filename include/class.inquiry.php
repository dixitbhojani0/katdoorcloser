<?php
  class Inquiry extends Functions
  {
  	...
  	// Public Varibale
  	public $id='';public $name='';public $email='';public $subject='';public $message='';public $created_date=''; 
  	public $valid_keys=array("id","name","email","subject","message","created_date");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="")
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Inquiry detail fetched successfully!!","ack_msg"=>"Inquiry detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Inquiry detail can not be fetch!","ack_msg"=>"Inquiry detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Inquiry detail cannot be fetch!","ack_msg"=>"Inquiry detail cannot be fetch!");
  			return $reply;
  		}
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countInquiryUs($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Inquiry sent successfully!!","ack_msg"=>"Inquiry sent successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Inquiry can't be sent!!","ack_msg"=>"Inquiry can't be sent!!");
  							return $reply;
  						}
  					}
  					else
  					...
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
  								...
  								$reply=array("ack"=>1,"developer_msg"=>"Inquiry detail updated successfully!!","ack_msg"=>"Inquiry detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Inquiry detail cannot be updated!!","ack_msg"=>"Inquiry detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countInquiryUs($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				$this->db->rp_delete($this->ctable,$delete_array['key']."=".$delete_array['value'],0);
              	$reply=array("ack"=>1,"developer_msg"=>"Inquiry deleted successfully!!","ack_msg"=>"Inquiry deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Inquiry cannot be fetch!","ack_msg"=>"Inquiry cannot be fetch!");
  			return $reply;
  		}		
  	}
      function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				$count=$this->countInquiryUs($active_array['key'],$active_array['value']);
  				if($count>=1)
  				{
  					$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
  					$reply=array("ack"=>1,"developer_msg"=>"Inquiry deatil status updated successfully!!","ack_msg"=>"Inquiry status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
  	...
  	function countInquiryUs($key,$value)
  	{		
  		$countInquiryUs = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countInquiryUs;	
  	}
