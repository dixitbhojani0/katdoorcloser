<?php
  class Blog extends Functions
  {
  	...
  	public $unique_column="blog_title";
  	// Public Varibale

  	public $id='';public $blog_title='';public $cid='';public $slug='';public $description='';public $image_path='';public $adate='';  public $author_name='';

  	public $valid_keys=array("id","cid","blog_title","slug","description","add_info","author_name","image_path","old_image_path","adate");
      ...
      if($this->rights['view_flag']==1)
  		{             
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Blog detail fetched successfully!!","ack_msg"=>"Blog detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{  
  				$reply=array("ack"=>0,"developer_msg"=>"Blog detail can not be fetch!","ack_msg"=>"Blog detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Blog detail cannot be fetch!","ack_msg"=>"Blog detail cannot be fetch!");
  			return $reply;
  		}  		
  	}
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			// {
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countBlog($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						if (isset($file["image_path"])) 
  						{
  							...
  							$fileName = 'blog_img_'.substr(sha1(time()), 0, 6).".".$extension;
  							$tempPath="../images/blog/".$fileName;
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Blog Detail inserted successfully!!","ack_msg"=>"Blog Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Blog Detail can't be inserted!!","ack_msg"=>"Blog Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
      ...
      if($this->rights['update_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countBlog($this->primary_column,$primary_key);
  					if($count>=1)
  					{
              $count=$this->duplicateProduct($this->unique_column,$unique_key,$primary_key);
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Blog Detail updated successfully!!","ack_msg"=>"Blog Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Blog Detail cannot be updated!!","ack_msg"=>"Blog Detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countBlog($delete_array['key'],$delete_array['value']);
        if($count>=1)
        {
  				$values=array("isDelete"=>1);
  				$this->db->rp_update($this->ctable,$values,"id='".$delete_array['value']."'",0);
        	$reply=array("ack"=>1,"developer_msg"=>"Blog deleted successfully!!","ack_msg"=>"Blog deleted successfully!!");
        	return $reply;
        }
        else
        {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Blog cannot be fetch!","ack_msg"=>"Blog cannot be fetch!");
  			return $reply;
  		}		
  	} 
  	...
  	{   
  		if($this->rights['update_flag']==1)
  		{ 
				$count=$this->countBlog($active_array['key'],$active_array['value']);
				if($count>=1)
				{
					$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
					$reply=array("ack"=>1,"developer_msg"=>"Blog status updated successfully!!","ack_msg"=>"Blog status updated successfully!!");
					return $reply;
				}
				else
				{
  	...
  	function countBlog($key,$value)
  	{		
  		$countBlog = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countBlog;	
  	}
  	function duplicateProduct($key,$value,$primary_key)
  	{		
  		$countBlog = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countBlog;	
  	}
