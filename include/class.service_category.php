<?php
  class ServiceCategory extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="service_category";
  	public $primary_column="id";
  	public $unique_column="name";
  	// Public Varibale
  	public $id='';public $name='';public $slug='';public $meta_descr='';public $description='';public $image_path='';public $image=''; 
  	public $valid_keys=array("id","name","slug","meta_descr","description","image_path","adate","old_image_path");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="",$isSubCategoryRequired=false,$isProductRequired=false)
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Service Category detail fetched successfully!!","ack_msg"=>"Service Category detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Service Category detail can not be fetch!","ack_msg"=>"Service Category detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Service Category detail cannot be fetch!","ack_msg"=>"Service Category detail cannot be fetch!");
  			return $reply;
  		}
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						if (isset($file["image_path"])) 
  						{
  							...
  							$fileName = 'category_img_'.substr(sha1(time()), 0, 6).".".$extension;
  							$tempPath="../images/service_category/".$fileName;
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Service Category inserted successfully!!","ack_msg"=>"Service Category inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Service Category  can't be inserted!!","ack_msg"=>"Service Category can't be inserted!!");
  							return $reply;
  						}
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Service Category detail updated successfully!!","ack_msg"=>"Service Category detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Service Category detail cannot be updated!!","ack_msg"=>"Service Category detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$reply=array("ack"=>1,"developer_msg"=>"Service Category deleted successfully!!","ack_msg"=>"Service Category deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Service Category cannot be fetch!","ack_msg"=>"Service Category cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				if($count>=1)
  				{
  					...
  					$reply=array("ack"=>1,"developer_msg"=>"Service Category status updated successfully!!","ack_msg"=>"Service Category status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
  	...
  	function favorite_category($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				if($count>=1)
  				{
  					$this->db->rp_update($this->ctable,array("isFav"=>$active_array['isFav']),$active_array['key']."=".$active_array['value'],0);
  					$reply=array("ack"=>1,"developer_msg"=>"Service Category status updated successfully!!","ack_msg"=>"Service Category status updated successfully!!");
  					return $reply;
  				}
  				else
