  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/gallery/".$file['image_path']['name'];
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						// $count=$this->duplicateProduct($this->unique_column,$unique_key,$primary_key);
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Gallery Detail updated successfully!!","ack_msg"=>"Gallery Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Gallery Detail cannot be updated!!","ack_msg"=>"Gallery Detail cannot be updated!!");
  								return $reply;
  							}
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$this->db->rp_update("gallery",$values,"cid=".$delete_array['value']."",0);
              	
  				$reply=array("ack"=>1,"developer_msg"=>"Gallery deleted successfully!!","ack_msg"=>"Gallery deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Gallery cannot be fetch!","ack_msg"=>"Gallery cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				if($count>=1)
  				{
  					...
  					$this->db->rp_update("gallery",array("isActive"=>$active_array['status']),"cid=".$active_array['value']."",0);
  					
  					$reply=array("ack"=>1,"developer_msg"=>"Gallery status updated successfully!!","ack_msg"=>"Gallery status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
