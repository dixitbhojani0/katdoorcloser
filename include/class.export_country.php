<?php
  class ExportCountry extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="export_country";
  	public $primary_column="id";
  	public $unique_column="name";
  	// Public Varibale
  	public $id='';public $name='';public $image_path=''; 
  	public $valid_keys=array("id","name","image_path","old_image_path");
  	...
  	function insert($detail=array(),$dup_check_array=array(),$file)
  	{
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countCountry($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/export_country/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Export Country inserted successfully!!","ack_msg"=>"Export Country inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Export Country  can't be inserted!!","ack_msg"=>"Export Country can't be inserted!!");
  							return $reply;
  						}
  						}
  					else
  		...
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Export Country detail cannot be fetch!","ack_msg"=>"Export Country detail cannot be fetch!");
  			return $reply;
  		}
  	}
  		...
  		//PRINT_R($detail);EXIT;
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					$count=$this->countCountry($this->primary_column,$primary_key);
  					if($count>=1)
  					{
  						...
  						if($isUpdated)
  						{
  							$reply=array("ack"=>1,"developer_msg"=>"Export Country detail updated successfully!!","ack_msg"=>"Export Country detail updated successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Export Country detail cannot be updated!!","ack_msg"=>"Export Country detail cannot be updated!!");
  							return $reply;
  						}
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countCountry($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				$this->db->rp_delete($this->ctable,$delete_array['key']."=".$delete_array['value'],0);
              	$reply=array("ack"=>1,"developer_msg"=>"Export Country deleted successfully!!","ack_msg"=>"Export Country deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Export Country cannot be fetch!","ack_msg"=>"Export Country cannot be fetch!");
  			return $reply;
  		}		
  	}
      ...
      function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				$count=$this->countCountry($active_array['key'],$active_array['value']);
  				if($count>=1)
  				{
  					$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
  					$reply=array("ack"=>1,"developer_msg"=>"Export Country status updated successfully!!","ack_msg"=>"Export Country status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
  	...
  	function countCountry($key,$value)
  	{		
  		$countCountry = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
  		return $countCountry;	
  	}
  	function duplicateCountry($key,$value,$primary_key)
  	{		
  		$countCountry = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND ".$this->primary_column."!=".$primary_key,0);
  		return $countCountry;	
  	}
