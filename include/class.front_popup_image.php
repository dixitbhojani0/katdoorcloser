<?php
  class FrontPopupImage extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="front_popup_image";
  	public $primary_column="id";
  	public $unique_column="title";
  	// Public Varibale
  	public $id='';public $title='';public $slug='';public $meta_descr='';public $description='';public $image_path='';public $adate=''; 
  	public $valid_keys=array("id","image_path","old_image_path","title","description","adate");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="",$isSubCategoryRequired=false,$isProductRequired=false)
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			if($result_r)
  			{
  				...
  				$reply=array("ack"=>1,"developer_msg"=>"Gallery detail fetched successfully!!","ack_msg"=>"Gallery detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
  				return $reply;
  			}
  			else
  			{
  
  				$reply=array("ack"=>0,"developer_msg"=>"Gallery detail can not be fetch!","ack_msg"=>"Gallery detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Gallery detail cannot be fetch!","ack_msg"=>"Gallery detail cannot be fetch!");
  			return $reply;
  		}
  				...
  				if(!empty($detail))
  				{
  					...
  					if($count<=0)
  					{
  						...
  						if(!in_array($extension, $allowedExts)){
  							...
  							$tempPath="../images/front_popup_image/".$file['image_path']['name'];
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Gallery Detail inserted successfully!!","ack_msg"=>"Gallery Detail inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Gallery Detail can't be inserted!!","ack_msg"=>"Gallery Detail can't be inserted!!");
  							return $reply;
  						}
  					}	
  					else
  	...
  	function update($detail=array(),$unique_key,$primary_key,$file)
  	{
  				...
  				if(!empty($detail)&& $primary_key!="")
  				{
  					...
  					if($count>=1)
  					{
  						// $count=$this->duplicateProduct($this->unique_column,$unique_key,$primary_key);
  							...
  							if($isUpdated)
  							{
      						    $reply=array("ack"=>1,"developer_msg"=>"Front Popup Image Detail updated successfully!!","ack_msg"=>"Front Popup Image Detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Front Popup Image Detail cannot be updated!!","ack_msg"=>"Front Popup Image Detail cannot be updated!!");
  								return $reply;
  							}
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Front popup Image Detail cannot be fetch!","ack_msg"=>"Front popup Image Detail cannot be fetch!");
  			return $reply;
  		}
  	...
  	function delete($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
              ...
              if($count>=1)
              {
              	...
              	$this->db->rp_update("front_popup_image",$values,"cid=".$delete_array['value']."",0);
              	
  				$reply=array("ack"=>1,"developer_msg"=>"Front popup Image deleted successfully!!","ack_msg"=>"Front popup Image deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Front Popup Image cannot be fetch!","ack_msg"=>"Front Popup Image cannot be fetch!");
  			return $reply;
  		}		
  	}
  	...
  	function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				if($count>=1)
  				{
  					...
  					$this->db->rp_update("front_popup_image",array("isActive"=>$active_array['status']),"cid=".$active_array['value']."",0);
  					
  					$reply=array("ack"=>1,"developer_msg"=>"Front Popup Image status updated successfully!!","ack_msg"=>"Front Popup Image status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
