<?php
  class Newsletter extends Functions
  {
  	public $detail=array();
  	public $db,$rights;
  	public $ctable="newsletter";
  	public $primary_column="id";
  	public $unique_column="email";
  	// Public Varibale
  	public $id='';public $email='';public $adate='';public $unique_code=''; 
  	public $valid_keys=array("id","email","unique_code","adate");
  	...
  	function view($where="",$required_columns=array(),$orderby="",$limit="")
  	{
          ...
          if($this->rights['view_flag']==1)
  		{
  			...
  			else
  			{
  				...
  				$reply=array("ack"=>0,"developer_msg"=>"Newsletter detail can not be fetch!","ack_msg"=>"Newsletter detail can not be fetch!");
  				return $reply;
  			}
  		}
  		else
  		{
  			...
  			$reply=array("ack"=>0,"developer_msg"=>"Newsletter detail cannot be fetch!","ack_msg"=>"Newsletter detail cannot be fetch!");
  			return $reply;
  		}
  		...
  		if($this->rights['insert_flag']==1)
  		{
  			...
  			if($validateKey['ack']==1)
  			{
  				...
  				if(!empty($detail))
  				{
  					...
  					$count=$this->countNewsletter($dup_check_array['key'],$dup_check_array['value']);
  					else
  					$count=0;	
  					...
  					if($count<=0)
  					{
  						...
  						$detail['unique_code']=$this->generateActivationCode();
  						...
  						if($inserted_id!=0)
  						{
  							...
  							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Newsletter inserted successfully!!","ack_msg"=>"Newsletter inserted successfully!!");
  							return $reply;
  						}
  						else
  						{
  							$reply=array("ack"=>0,"developer_msg"=>"Newsletter inserted can't be inserted!!","ack_msg"=>"Newsletter  can't be inserted!!");
  							return $reply;
  						}
  					}
  					else
  					...
  					if($count>=1)
  					{
  						...
  						if($count<=0)
  						{
  							...
  							if($isUpdated)
  							{
  								...
  								$reply=array("ack"=>1,"developer_msg"=>"Newsletter detail updated successfully!!","ack_msg"=>"Newsletter detail updated successfully!!");
  								return $reply;
  							}
  							else
  							{
  								$reply=array("ack"=>0,"developer_msg"=>"Newsletter detail cannot be updated!!","ack_msg"=>"Newsletter detail cannot be updated!!");
  								return $reply;
  							}
  						}
  						else
  	...
  	function delete($delete_array)
  	{
  		...
  		{
  			$count=$this->countNewsletter($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				$this->db->rp_delete($this->ctable,$delete_array['key']."=".$delete_array['value'],0);
              	$reply=array("ack"=>1,"developer_msg"=>"Newsletter deleted successfully!!","ack_msg"=>"Newsletter deleted successfully!!");
              	return $reply;
              }
              else
              {
  		...
  		{
  			$this->db->rp_location("access_denied.php?msg=insert");
  			$reply=array("ack"=>0,"developer_msg"=>"Newsletter cannot be fetch!","ack_msg"=>"Newsletter cannot be fetch!");
  			return $reply;
  		}		
  	}
  	
  	
  	function unsubscribe_newsletter($delete_array)
  	{
  		if($this->rights['delete_flag']==1)
  		{
  			$count=$this->countNewsletter($delete_array['key'],$delete_array['value']);
              if($count>=1)
              {
  				...
  				$this->db->rp_delete($this->ctable,$delete_array['key']."='".$delete_array['value']."'",0);
              	$reply=array("ack"=>1,"developer_msg"=>"Newsletter deleted successfully!!","ack_msg"=>"Newsletter deleted successfully!!","email"=>$email,"name"=>$name);
              	return $reply;
              }
              else
              {
      ...
      function active($active_array)
  	{
  		if($this->rights['update_flag']==1)
  		{
  				...
  				$count=$this->countNewsletter($active_array['key'],$active_array['value']);
  				if($count>=1)
  				{
  					$this->db->rp_update($this->ctable,array("isActive"=>$active_array['status']),$active_array['key']."=".$active_array['value'],0);
  					$reply=array("ack"=>1,"developer_msg"=>"Newsletter status updated successfully!!","ack_msg"=>"Newsletter status updated successfully!!");
  					return $reply;
  				}
  				else
  				{
  	...
  	function countNewsletter($key,$value)
  	{		
  		$countNewsletter = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."'",0);
