<?php
  class Admin extends Functions
  {
  	...
  	function update($detail=array(),$primary_key="id")
  	{
  	...
  	function getAdminDetail($user_id=0,$required_columns=array())
  	{
  		...
  		if($user_id!=0)
  		{
  			...
  			if($result)
  			{
  				...
  				if($detail['image_path']['ack']==1 && file_exists(str_ireplace('..', SITE_PATH, $detail['image_path']['result'][0]['url'])))
  				{
      ...
      function getSellerDetail($user_id=0,$required_columns=array())
  	{
  		$required_columns=$this->getRequiredColumns($required_columns);
  		if($user_id!=0)
  		{
  			$where="id='".$user_id."'";
  			$result=$this->rp_getData($this->ctableSeller,$required_columns,$where);
  			if($result)
  			{
  				$detail=mysqli_fetch_assoc($result);
  				$detail['image_path']=$this->media->getMedia(array("mid"=>array($detail['image_path'])));
  	...
  	function getAdmins($user_id=0,$required_columns=array())
  	{
  		...
  		if($user_id!=0)
  		{
  			...
  			if($result_admins)
  			{
  				while($detail=mysqli_fetch_assoc($result_admins))
  				{
  					$detail['countFollower']=0;
  					$detail['countFollowing']=$this->db->rp_getTotalRecord("seller_follower","follower_id='".$user_id."'");
  					$detail['countNotification']=$this->db->rp_getTotalRecord("notification","1=1");
...
<?php
  ...
  {
  	$result=$this->rp_getData(CTABLE_ADMIN,"*","id='".$_SESSION[SITE_SESS.'_ADMIN_SESS_ID']."'");
  	if($result)
  	{
